mkdir -p "lib/photos"
cat > "lib/photos/moderate-image.ts" << 'ENDOFFILE'
/**
...
mkdir -p "lib/photos"
cat > "lib/photos/upload-avatar.ts" << 'ENDOFFILE'
...
mkdir -p "lib/photos"
cat > "lib/photos/upload-event-photo.ts" << 'ENDOFFILE'
import type { createAdminClient } from "@/lib/supabase/admin";
import { moderateImage } from "@/lib/photos/moderate-image";
...
/**
 * Подтверждение создания бизнес-события — организатору в Telegram сразу
 * после публикации (см. явный запрос пользователя): название, дата и
 * время, короткое описание (если есть), сколько человек и условия участия.
 */
function formatBusinessEventCreatedMessage(input: CreateEventInput): string {
  ...
  const conditionsLabel =
    input.businessPricingType === "ticket"
      ? `Билет: ${input.businessPricingDetails}`
      : input.businessPricingType === "custom"
        ? input.businessPricingDetails ?? "Свои условия"
        : "Бесплатно";

  const lines = [
    `Событие «${input.title}» создано ✅`,
    `${dateLabel}, ${timeLabel}`,
...
export function CreateEventWizard() {
  ...
  function handleBusinessPhotoChange(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => setBusinessPhotoBase64(reader.result as string);
    reader.readAsDataURL(file);
  }
