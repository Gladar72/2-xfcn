...
// Точность округления координат для "одного и того же места" — 4 знака
// после запятой ≈ 11 метров, этого достаточно, чтобы объединить одно
// подтверждённое место (даже с небольшими расхождениями геокодирования),
// но не смешать соседние разные здания.
const SAME_PLACE_PRECISION = 4;
...
function createClusterButton(count: number, onClick: () => void): HTMLButtonElement {
  ...
  button.style.cssText = `
    box-sizing:border-box;width:${size}px;height:${size}px;border:4px solid transparent;
    border-radius:50%;
    background:linear-gradient(#fff,#fff) padding-box,linear-gradient(135deg,#6c3bff,#8a5cff 45%,#ff8a2a) border-box;
    color:#111;font:800 ${isThreeChars ? 19 : 20}px/1 var(--font-onest),Onest,Arial,sans-serif;
