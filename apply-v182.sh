...
.mesto{
 ...
 /* Тот же проверенный приём, что и на странице "Заявки": растягиваем на
    весь экран, а дублирующий отступ общего layout (pb-24) компенсируем
    отрицательным margin на обёртке в page.tsx — см. там. */
...
.m-page{width:100%;max-width:480px;margin-inline:auto;padding:calc(16px + var(--m-safe-top)) calc(var(--m-pad) + var(--m-safe-right)) calc(var(--m-nav-height) + var(--m-safe-bottom) + 14px) calc(var(--m-pad) + var(--m-safe-left))}
...
.m-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));align-items:stretch;gap:14px;margin-top:14px}
...
.m-cancel{margin-top:14px;min-height:58px;background:#fff;text-align:center;gap:10px;padding:7px 16px;box-shadow:0 10px 30px #3714730C}
